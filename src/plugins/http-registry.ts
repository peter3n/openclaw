...
export function registerPluginHttpRoute(params: {
  ...
  if (existingIndex >= 0) {
    ...
    const mismatchedOwner = canonicalMatches.find(
      (route) => normalizeOptionalString(route.pluginId) !== replacementOwner,
    );
    if (mismatchedOwner) {
      return rejectRegistration(
        `plugin: route replacement denied for ${normalizedPath} (${routeMatch})${suffix}; owned by ${mismatchedOwner.pluginId ?? "unknown-plugin"}`,
      );
    }
